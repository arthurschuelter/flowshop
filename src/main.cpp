...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < v_jobs.size(); ++i) {
        int num_jobs = v_jobs[i];
        int num_batches = num_jobs + num_machines;
        int iterations = 10;
        ...
        try {
            ...
            for (int it = 0; it < iterations; it++) {
                ...
                std::cout << "(" << it << ") " << results_liao->model->get(GRB_DoubleAttr_ObjVal);
                std::cout << " | " << results_mine->model->get(GRB_DoubleAttr_ObjVal) << std::endl;
            ...
            std::string filepath = "./data-analysis/results/results-" + suffix[i] + ".csv";
            FILE *fp=freopen(filepath.c_str(),"w",stdout);
